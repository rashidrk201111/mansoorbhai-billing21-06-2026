...
export function Invoices() {
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    try {
      ...
      const wasPreviouslySentOrPaid = invoice.status === 'sent' || invoice.status === 'paid';
      const isNowSentOrPaid = status === 'sent' || status === 'paid';

      if (isNowSentOrPaid && !wasPreviouslySentOrPaid) {
        ...
        for (const item of invoiceItems || []) {
          ...
          const { error: movementError } = await supabase
            ...
            .insert([{
              ...
              type: 'out',
              quantity: item.quantity,
              reason: `Invoice ${invoice.invoice_number} marked as ${status}`,
              created_by: profile?.id,
            }]);
