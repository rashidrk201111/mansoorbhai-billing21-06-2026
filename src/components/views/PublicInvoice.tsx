...
export function PublicInvoice() {
  ...
  const handlePrint = () => {
    // Toggle GST off for printing
    const currentShowGst = showGst;
    setShowGst(false);
    
    // Let the UI update before printing
    setTimeout(() => {
      window.print();
      // Restore the original GST setting after printing
      setTimeout(() => setShowGst(currentShowGst), 500);
    }, 100);
  };
        ...
        <div id="invoice-content" className="bg-white rounded-lg shadow-lg p-8">
              ...
              <div className="flex items-start gap-6">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-600 space-y-1">
                    ...
                    {showGst && companyProfile?.gst_number && <p>GSTIN: {companyProfile.gst_number}</p>}
                  </div>
                </div>
              </div>
          ...
          <div className="mb-6">
            ...
            <div className="text-slate-900">
              ...
              {showGst && invoiceData.customer.gstin && <p className="text-sm">GSTIN: {invoiceData.customer.gstin}</p>}
            </div>
          </div>
          ...
          <table className="w-full mb-6 table-fixed">
            <colgroup>
              <col className="w-2/5" />
              <col className="w-1/6" />
              {showGst ? <col className="w-1/6" /> : null}
              <col className="w-1/4" />
            </colgroup>
            <thead>
              <tr className="bg-slate-100 border-b-2 border-slate-300">
                ...
                {showGst && <th className="text-right p-3 font-semibold text-slate-700">Rate</th>}
                <th className="text-right p-3 font-semibold text-slate-700">Amount</th>
              </tr>
            </thead>
            <tbody>
              {invoiceData.items.map((item: any, index: number) => {
                const amount = showGst ? item.total_price : (item.unit_price * item.quantity);
                return (
                  <tr key={index} className="border-b border-slate-200">
                    ...
                    <td className="text-right p-3 text-slate-900 font-medium">
                      {formatINR(amount)}
                    </td>
                  </tr>
                );
              ...
              {!showGst && (
                <>
                  <tr className="border-t-2 border-slate-300 font-medium">
                    <td className="p-3 text-right text-slate-700" colSpan={2}>
                      Total Quantity:
                    </td>
                  ...
                  <tr className="border-t border-slate-200 font-medium">
                    ...
                    <td className="p-3 text-right text-slate-700 font-bold">
                      {formatINR(invoiceData.items.reduce((sum: number, item: any) => 
              ...
              {showGst && (
                <tr className="border-t-2 border-slate-300 font-medium">
                  <td className="p-3 text-right text-slate-700" colSpan={2}>
                    Total Qty: {invoiceData.items.reduce((sum: number, item: any) => sum + item.quantity, 0)}
                  </td>
                  <td></td>
                  <td className="p-3 text-right text-slate-700 font-bold">
                    {formatINR(invoiceData.items.reduce((sum: number, item: any) => 
          ...
          <div className="flex justify-end mb-6">
            <div className="w-64">
              ...
              <div className="flex justify-between py-3 border-t-2 border-slate-300 font-bold text-lg text-slate-900">
                <span>Total{showGst ? ' (Incl. Tax)' : ' (Excl. Tax)'}:</span>
                <span>{formatINR(showGst ? invoiceData.total_amount : invoiceData.subtotal)}</span>
              </div>
            </div>
          </div>
